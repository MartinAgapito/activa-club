...
- [x] El backend extrae el `username` del `AccessToken` y llama a `AdminUserGlobalSignOut` de Cognito, invalidando todos los tokens activos de esa cuenta (no solo la sesión actual).
- [x] Tras la llamada exitosa al backend, el frontend limpia el store de Zustand (tokens, datos del usuario) y redirige al socio a `/login`.
...
- [x] Tras el logout, si el usuario intenta navegar a una ruta protegida con los tokens invalidados, el autorizador de API Gateway rechaza la petición (HTTP 401) y el frontend redirige a `/login`.
...
- [x] El Lambda extrae el `username` del `AccessToken` y llama a `AdminUserGlobalSignOut` de Cognito correctamente.
